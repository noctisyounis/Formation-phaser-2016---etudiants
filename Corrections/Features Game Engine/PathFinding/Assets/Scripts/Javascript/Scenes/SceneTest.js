...
function SceneTest() {
	...
	this.Start = function() {
		if (!this.started) {
			...
			//this.GameObjects.push(new IA());

			//console.log(ps);

			this.grid = new Grid(500,10);
			console.log(this.grid);
			

			this.oldGrid = this.grid.obstacle.slice(0);

			this.PathFinding = new PathFinding(this.grid.obstacle,this.grid.c,this.grid.c);
			this.PathFinding.walkableTiles = [0,2,3,4,8];
	...
	this.GUI = function() {
		if (!Application.GamePaused) {
			...
			if (Input.MouseClick) {
				var p = this.grid.getMousePosition();

				var chemin = this.PathFinding.Process(this.startPoint,p);


				this.grid.obstacle = this.oldGrid.slice(0);
				this.grid.obstacle[indexFromCoord(this.startPoint.x,this.startPoint.y,this.grid.c)] = 0;
				this.grid.obstacle[indexFromCoord(p.x,p.y,this.grid.c)] = -1;

				for (v of chemin ) {
					var i = indexFromCoord(v.x,v.y,this.grid.c);
					if (!i) {
						continue;
					}
					this.grid.obstacle[i] = 8;
				}
