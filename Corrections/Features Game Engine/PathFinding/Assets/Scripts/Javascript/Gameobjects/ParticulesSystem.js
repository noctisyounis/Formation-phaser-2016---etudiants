...
function ParticulesSystem() {
	this.name = "ParticulesSystem";
	this.enabled = true;
	this.started = false;
	this.rendered = true;
	
	this.Transform = {};
	this.Transform.position = new Vector(canvas.width/2,canvas.height/2);
	...
	this.Start = function() {
		if (!this.started) {
			...
			var e = new Emitter(this.Transform.position,new Vector(2,2),150,8,200);
			this.emitters.push(e);

			this.fields.push(new Field(new Vector(1000,465, -40)))
	...
	this.Update = function() {
		if ( this.enabled ) {
			...
			ctx.fillStyle = this.fields[0].drawColor;
			ctx.fillRect(this.fields[0].position.x,this.fields[0].position.y,5,5)
		}
		this.GUI();	
	};
	this.GUI = function() {
