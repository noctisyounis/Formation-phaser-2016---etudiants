...
function IA() {
	...
	this.Start = function() {
		if (!this.started) {
			...
			this.behaviour = new BehaviourTree("test");
			this.behaviour.createNode("start", true);
			this.behaviour.createNode("move");
			this.behaviour.createNode("start");
	...
	this.Update = function() {
		if ( this.enabled ) {
			//ctx.fillStyle = '#006DDB';
			//ctx.fillRect(this.Transform.position.x, this.Transform.position.x, this.Transform.size.x, this.Transform.size.y);
		}
		this.GUI();	
	};
	this.GUI = function() {
