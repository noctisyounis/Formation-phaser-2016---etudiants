...
function BoyScene1() {
	...
	this.Awake = function() {
		console.log('%c System:BoyScene1 ' + this.name + " Created !", 'background:#222; color:#b00b55');
	};
	this.Start = function() {
		if (!this.started) {
			// operation start
			...
			this.Transform.size.x = 101;
			this.Transform.size.y = 80;
			...
			this.started = true;
			console.log('%c System:BoyScene1 ' + this.name + " Started !", 'background:#222; color:#bada55');
	...
	this.Update = function() {
		if ( this.enabled ) {
			...
			this.Physics.x = this.Transform.position.x;
			this.Physics.y = this.Transform.position.y;
