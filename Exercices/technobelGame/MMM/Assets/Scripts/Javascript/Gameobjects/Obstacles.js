...
function Obstacles() {
	...
	this.Renderer = {
		...
		Draw: function() {
			...
			{
				var scaledSizeX = this.that.size.x*this.that.scale.x;
				var scaledSizeY =  300;	//this.that.size.y*this.that.scale.y;
				ctx.drawImage(this.Material.Source,
								this.that.position.x-this.that.pivot.x*scaledSizeX,
								this.that.position.y-this.that.pivot.y*scaledSizeY,
								scaledSizeX,
	...
	this.Awake = function() {
		console.log('%c System:Obstacles ' + this.name + " Created !", 'background:#222; color:#b00b55');
	};
	this.Start = function() {
		if (!this.started) {
			// operation start
			...
			this.Renderer.Material.Source = Images["woodBlock"];
			this.Transform.position.x = Math.Random.RangeInt(0, 1000);
			this.Transform.position.y = 0;
			this.Transform.size.x = 90;
			...
			this.started = true;
			console.log('%c System:Obstacles ' + this.name + " Started !", 'background:#222; color:#bada55');
		}
		this.Update();
	};
