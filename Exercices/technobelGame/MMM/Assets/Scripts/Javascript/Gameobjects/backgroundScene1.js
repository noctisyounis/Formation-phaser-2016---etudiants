...
function BackgroundScene1() {
	this.name = "BackgroundScene1";
	this.enabled = true;
	this.started = false;
	this.rendered = true;
	...
	this.Transform.size = new Vector(0, 0);
	...
	this.Renderer = {
		...
		Draw: function() {
			...
			else 
			{
				...
				var scaledSizeX = this.that.size.x*this.that.scale.x;
				var scaledSizeY = 1200; //this.that.size.y * this.that.scale.y;
				ctx.drawImage(this.Material.Source,
								this.that.position.x-this.that.pivot.x*scaledSizeX,
								this.that.position.y-this.that.pivot.y*scaledSizeY,
								scaledSizeX,
	...
	this.Awake = function() {
		console.log('%c System:BackgroundScene1 ' + this.name + " Created !", 'background:#222; color:#b00b55');
	};
	this.Start = function() {

		if (!this.started) {
			// operation start
			this.Renderer.Material.Source = Images["sky"];
			// console.log(Images["woodBlock"])
			this.Transform.position.x = 0;
			this.Transform.position.y = 0;
			this.Transform.size.x = canvas.width;
			this.Transform.size.y = canvas.heigth;
			this.Transform.scale.x = 2;
			this.Transform.scale.y = 2;
			this.Transform.pivot.x = 0;
			this.Transform.pivot.y = 0;
			this.started = true;
			console.log('%c System:BackgroundScene1 ' + this.name + " Started !", 'background:#222; color:#bada55');
		}
		this.Update();
	};
