...
function Vie() {
	...
	this.Renderer = {
		...
		Draw: function() {
			...
			{
				var scaledSizeX = this.that.size.x*this.that.scale.x;
				var scaledSizeY =  100;	//this.that.size.y*this.that.scale.y;
				ctx.drawImage(this.Material.Source,
								this.that.position.x-this.that.pivot.x*scaledSizeX,
								this.that.position.y-this.that.pivot.y*scaledSizeY,
								scaledSizeX,
	...
	this.Awake = function() {
		console.log('%c System:Vie ' + this.name + " Created !", 'background:#222; color:#b00b55');
	};
	this.Start = function() {
		if (!this.started) {
			// operation start
			...
			this.Transform.position.x = canvas.width - 50 * nombreDeVies;
				nombreDeVies--;
			...
			this.started = true;
			console.log('%c System:Vie ' + this.name + " Started !", 'background:#222; color:#bada55');
		}
		this.Update();
	};
