...
function Character() {
	...
	this.Start = function() {
		if (!this.started) {
			...
			this.Transform.position.y = canvas.height-(115/2);
			this.Transform.size.x = 67/2;
			this.Transform.size.y = 88/2;
		...
		if ( this.enabled ) {
			...
			//if (Input.KeysDown[37] || Input.KeysDown[38] || Input.KeysDown[39] || Input.KeysDown[40]) {}
				...
				if (Input.KeysDown[37]) {
					this.Transform.position.x -= walkSpeed;					
				}
                if (Input.KeysDown[38]) {
                    this.Transform.position.y -= walkSpeed;
                }
                ...
                for(var i in Application.LoadedScene.GameObjects){
					...
					if(go.Renderer.id == 2){//gem
						var current;
						var box2 = {
							x:go.Transform.position.x,
							y:go.Transform.position.y,
							w:go.Transform.size.x,
							h:go.Transform.size.y
						}
						var res = Physics.BoxBoxCollision(box1, box2);
						if(res){
							current = go;
							current.Transform.scale.x = 0;
							current.Transform.scale.y = 0;
						}
					}
				}
			this.Renderer.Draw();
