...
function TweenObjectSins() {
	this.name = "TweenObjectSins";
	this.enabled = true;
	this.started = false;
	this.rendered = true;
	...
	this.Awake = function() {
		console.log('%c System:TweenObjectSins ' + this.name + " Created !", 'background:#222; color:#b00b55');
	};
	this.Start = function() {
		if (!this.started) {
			// operation start
			this.Transform.relativePosition = new Vector(100, 600);
	...
	this.Update = function() {
		...
		if(this.Move.start && !this.Move.pause ) {
			...
			this.Transform.relativePosition.y = 600 + 100 * Math.sin(i);
			}
