...
function SelectionItem(x,y,w,h,imgName) {
	this.name = imgName;
	this.enabled = true;
	this.started = false;
	this.rendered = true;
	...
	this.Update = function() {
		if ( this.enabled ) {
			...
			} else ctx.fillStyle = "rgba(200,175,15,0.75)";
			
			ctx.RoundedBox( this.Transform.position.x, this.Transform.position.y+15, this.Transform.size.x, this.Transform.size.y, 15);
			this.Renderer.Draw();
		}
		this.GUI();	
	};
	...
	this.onClicked = function() {
		this.Physics.countHovered ++;
		Application.PlayerImg = this.name;
		Application.Level = 1;
		Scenes["Level1"] = new Level(LevelData[Application.Level].map,LevelData[Application.Level].items);
		Application.LoadedScene = Scenes["Level1"];
	}
