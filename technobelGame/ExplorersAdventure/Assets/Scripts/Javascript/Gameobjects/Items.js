...
function Items(mapX,mapY,x,y,w,h,imgName) {
	this.name = imgName;
	this.enabled = true;
	this.started = false;
	this.rendered = true;
	...
	this.Transform = {};
	this.Transform.position = new Vector(x+0.25*w,y+0.15*h);
	this.Transform.mapPosition = new Vector(mapX,mapY);
	...
	this.Update = function() {
		if ( this.enabled ) {
			if (Application.Player.Transform.mapPosition.x == this.Transform.mapPosition.x && Application.Player.Transform.mapPosition.y == this.Transform.mapPosition.y ) {
				this.HitPlayer();
			}
			this.Renderer.Draw();
		}
