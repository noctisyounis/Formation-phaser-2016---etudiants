...
function Player(posX, posY, sprite, clickable) {
	this.name = "Player";
	this.enabled = true;
	this.started = false;
	this.rendered = true;
	...
	this.Physics.Clickable = clickable || false;
	...
	this.Start = function() {
		if (!this.started) {
			...
			else if(sprite === "catGirl")
			{
				this.Renderer.Material.Source = Images["Character Cat Girl"];
			}
			else if(sprite === "hornGirl")
			{
				this.Renderer.Material.Source = Images["Character Horn Girl"];
			}
			else if(sprite === "pinkGirl")
			{
				this.Renderer.Material.Source = Images["Character Pink Girl"];
			}
			else if(sprite === "princess")
			{
				this.Renderer.Material.Source = Images["Character Princess Girl"];
			}
	...
	this.Update = function() {
		if ( this.enabled )
		{
			if (Input.KeysDown[37])
			{
				console.log('gauche');
				currentDirection = 1;
				Physics.TileCollision(this.map, (10,8), this.Transform.position, currentDirection);
			}
	...
	this.onHover = function() {
		...
		ctx.fillStyle = "rgba(254, 202, 126, 0.4)";
		ctx.fillRect(this.Transform.position.x - this.Transform.pivot.x * (this.Transform.size.x*this.Transform.scale.x), this.Transform.position.y - this.Transform.pivot.y * (this.Transform.size.y*this.Transform.scale.y), this.Transform.size.x, this.Transform.size.y);
	}
	this.onClicked = function() {
		this.Physics.countHovered ++;
		Game.imageCharacter = sprite;
		Application.LoadedScene = Scenes["Level01"];		
	}
