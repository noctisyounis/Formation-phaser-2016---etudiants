 ...
 *
 * */
function ItemPoint(_x,_y,_scale) 
{
	this.name = "ItemPoint";
	this.enabled = true;
	this.started = false;
	...
	this.Transform.Size = new Vector(32,30);
	this.Transform.RelativeScale = new Vector(_scale,_scale);
	this.Transform.Scale = new Vector(_scale,_scale);
	...
	this.Renderer = 
	{
		...
		Draw: function() 
		{
			...
			console.log('Scale',this.That.Scale.x,this.That.Scale.y);
			ctx.save();
	...
	this.Start = function() 
	{
		if (!this.started) {
			...
			this.Renderer.Material.Source = Images["ItemPoint"];
