...
function CirclePhysics1() 
{
	this.name = "CirclePhysics1";
	this.enabled = true;
	this.started = false;
	this.rendered = true;
	...
	this.Start = function() 
	{
		if (!this.started) {
			...
			this.Transform.Radius = 50;
			this.SetPosition(300,100);
			this.SetScale(1,1);
			this.Physics.Collider = new Circle(300,100,60);
	...
	this.Update = function() 
	{
		...
		ctx.beginPath();
		ctx.arc(this.Transform.Position.x, this.Transform.Position.y, this.Transform.Radius, 0, Math.PI * 2);
		ctx.closePath();
		ctx.fill();
