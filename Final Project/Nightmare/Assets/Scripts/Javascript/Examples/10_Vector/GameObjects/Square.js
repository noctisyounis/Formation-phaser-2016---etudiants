...
function SquareVector() {
	this.name = "SquareVector";
	this.enabled = true;
	this.started = false;
	this.rendered = true;
	...
	this.Start = function() {
		if (!this.started) {
			// operation start
			this.Transform.relativePosition = new Vector(500, 500);
			this.Transform.size = new Vector(200, 200);
	...
	this.Update = function() {
		...
		if (Physics.CheckCollision(Input.MousePosition, this.box)) {
			...
			if (Input.mouseLongClick) {
					...
					if(!this.getClic.isClick) {
						this.getClic.isClick = true;
						this.getClic.x = Input.MousePosition.x;
						this.getClic.y = Input.MousePosition.y;
						this.getClic.dx = Math.abs(Input.MousePosition.x - this.Transform.relativePosition.x); 
						this.getClic.dy = Math.abs(Input.MousePosition.y - this.Transform.relativePosition.y);	
						console.log(this.getClic.dy + " / " + this.getClic.dx)					
						}

				this.Transform.relativePosition.x =  Input.MousePosition.x - this.getClic.dx;
				this.Transform.relativePosition.y =   Input.MousePosition.y - this.getClic.dy;
				this.box.x = this.Transform.relativePosition.x;
				this.box.y = this.Transform.relativePosition.y;
				this.text = " mouseX: " + Input.MousePosition.x.toString() + 
						" mouseY: " + Input.MousePosition.y.toString() + "|"+
						" objX: " + this.Transform.relativePosition.x.toString() +
						" objY: " + this.Transform.relativePosition.y.toString() ;

			} else {
				this.getClic.isClick = false;
			}
		}
		// this.Transform.relativePosition.x +=0.1;
