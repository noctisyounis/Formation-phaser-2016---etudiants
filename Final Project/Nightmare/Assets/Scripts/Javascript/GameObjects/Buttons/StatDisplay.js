 ...
 *
 * */
function StatDisplay(_x,_y,_scaleX,_scaleY) 
{
	...
	this.Transform.Size = new Vector(300,270);
	...
	this.Update = function() 
	{
		...
		ctx.stroke();
		ctx.fillStyle = "#000";
		var size = this.Transform.Size.y * this.Transform.Scale.y / 10;
		ctx.font = size*this.Transform.Scale.x + "px Consolas";
		ctx.textBaseline = "middle";
		ctx.textAlign = "center";
		var stats = PlayersConfig[Application.LoadedScene.playerIndex];
		var posY = this.Transform.Position.y - this.Transform.Size.y * this.Transform.Scale.y * this.Transform.Pivot.y + size * 0.75;
		for (n in stats) 
		{
			if (n == "spriteName") continue;
			ctx.fillText(n + " :", this.Transform.Position.x , posY );
			posY += size;
			ctx.fillText(stats[n], this.Transform.Position.x , posY );
			posY += 1.5 * size;
		}
