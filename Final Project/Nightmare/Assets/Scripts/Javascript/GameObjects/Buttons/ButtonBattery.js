 ...
 *
 * */
function ButtonBattery(_x,_y,_scaleX,_scaleY) 
{
	this.name = "ButtonBattery";
	this.enabled = true;
	this.started = false;
	this.rendered = true;
	...
	this.Transform.Size = new Vector(300,100);
	...
	this.Start = function() 
	{
		if (!this.started) {
			...
			{
				this.Physics.Collider.Position.x = this.Transform.Position.x - this.Transform.Size.x * this.Transform.Scale.x * this.Transform.Pivot.x;
				this.Physics.Collider.Position.y = this.Transform.Position.y - this.Transform.Size.y * this.Transform.Scale.y * this.Transform.Pivot.y;
				this.Physics.Collider.Size.x = this.Transform.Size.x * this.Transform.Scale.x;
				this.Physics.Collider.Size.y = this.Transform.Size.y * this.Transform.Scale.y;
			}
	...
	{
		if (this.Physics.countHovered > 0) ctx.fillStyle = "rgba(220,250,0,0.5)";
		else ctx.fillStyle = "rgba(255,255,255,0.25)";

		ctx.lineWidth = 5;
		ctx.strokeStyle = "#000";
		ctx.RoundedBox(this.Transform.Position.x - this.Transform.Size.x * this.Transform.Scale.x * this.Transform.Pivot.x,
		...
		ctx.stroke();
		ctx.fillStyle = "#000";
		ctx.font = 25*this.Transform.Scale.x + "px Consolas";
		ctx.textBaseline = "bottom";
		ctx.textAlign = "center";
		ctx.fillText("Battery Mode :", this.Transform.Position.x, this.Transform.Position.y);

		ctx.textBaseline = "top";
		if (Application.batteryMode)
		{
			ctx.fillStyle = "#00E92C";
			ctx.fillText("ON", this.Transform.Position.x, this.Transform.Position.y);
		}
		else 
		{
			ctx.fillStyle = "#EF0707";
			ctx.fillText("OFF", this.Transform.Position.x, this.Transform.Position.y);
		}
	...
	this.onClicked = function() 
	{
		...
		Application.batteryMode = !Application.batteryMode;
		this.Physics.countHovered ++;
	}
	/**
