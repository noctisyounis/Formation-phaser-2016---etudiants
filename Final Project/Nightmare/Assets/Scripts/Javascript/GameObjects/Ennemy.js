...
function Ennemy(x,y,type) 
{
	...
	this.Start = function() 
	{
		if (!this.started) 
		{
			...
			//this.Renderer.isSpriteSheet = false;
			//this.Renderer.Material.Source = Images["Enemy Bug"];

			this.SetSpriteSheet(Images["monster"+this.monsterID], new Vector(32,32), 0.5)
			...
			var alpha = Math.atan2( ( canvas.height / 2 - this.Transform.Position.y ), ( canvas.width / 2 - this.Transform.Position.x ) );
			...
			this.orientation = Math.RadianToDegree(alpha);

			// 0 = left
			//console.log(this.orientation)
			if (this.orientation >= 75 && this.orientation < 135) 
			{
				this.Renderer.Animation.Current = this.Renderer.Animation.Animations[0];
				this.Transform.angle = (this.orientation - 90) * 0.6;
			} 
			else if (this.orientation >= 135 && this.orientation < 270) 
			{
				this.Renderer.Animation.Current = this.Renderer.Animation.Animations[1];
				this.Transform.angle = (this.orientation - 180) * 0.6;
			}
			else if (this.orientation >= 270 && this.orientation < 315) 
			{
				this.Renderer.Animation.Current = this.Renderer.Animation.Animations[3];
				this.Transform.angle = (this.orientation - 270) * 0.6;
			}
			else if ( (this.orientation >= 315 && this.orientation <= 360) || (this.orientation >= 0 && this.orientation < 75) ) 
			{
				this.Renderer.Animation.Current = this.Renderer.Animation.Animations[2];
				if (this.orientation < 90) 
				{
					this.Transform.angle = this.orientation * 0.6;
				}
				else
				{
					this.Transform.angle = (this.orientation-360) * 0.6;
				}	
			}
	...
	this.GUI = function() 
	{
		if (this.hp > 1) 
		{
			...
			ctx.fillRect(this.Transform.Position.x - this.Transform.Size.x * this.Transform.Scale.x * this.Transform.Pivot.x,
				this.Transform.Position.y - this.Transform.Size.y * this.Transform.Scale.y * this.Transform.Pivot.y - 5, 
				this.hp / this.maxHp * this.Transform.Size.x * this.Transform.Scale.x ,
				4)
		}
	}
	...
	this.moveToCenter = function() 
	{
		...
		this.Transform.RelativePosition.x += this.speed * Time.deltaTime * Math.cos(Math.DegreeToRadian(this.orientation));
		this.Transform.RelativePosition.y += this.speed * Time.deltaTime * Math.sin(Math.DegreeToRadian(this.orientation));
	}
	this.CheckOppositeOrientation = function () 
	{
		var r = this.GetOrientation();
		//console.log(r);

		if (r > 1 - (Application.LoadedScene.Player.fieldOfView/200) ) 
		{
			return true;
		}
		return false;
	...
	this.GetOrientation = function function_name(argument) {
		var v = new Vector(1,1);
		var normPlayer = v.FromAngle(Math.DegreeToRadian(Application.LoadedScene.Player.Transform.angle - 90)).Normalize();
		var normEnnemy = v.FromAngle(Math.DegreeToRadian(this.orientation)).Normalize();

		var r = Math.DotProduct(normPlayer, normEnnemy);
