...
function Asteroid() 
{
	...
	this.Transform.Size = new Vector(762,815);
	this.Transform.RelativeScale = new Vector(1,1);
	this.Transform.Scale = new Vector(1,1);
	this.Transform.Pivot = new Vector(.5,.5);
	this.Transform.angle = 0;
	this.Transform.CircleColliser = new Circle(this.Transform.RelativePosition.x,this.Transform.RelativePosition.y,this.Size);
	...
	this.Start = function() 
	{
		if (!this.started)
		{
			...
			var r = 1000;
			var r1 = 400;
			this.Renderer.Material.Source = Images["Asteroid"];
			this.SetScale(.2,.2);
			this.Transform.RelativePosition = Math.Random.InCircle(new Circle(canvas.width/2, canvas.height/2, r));
			this.Transform.angle = Math.RadianToDegree(Math.atan2( Math.Random.RangeInt((canvas.height*.5 - r1), (canvas.height*.5 + r1), true) - this.Transform.RelativePosition.y, Math.Random.RangeInt((canvas.width*.5 - r1), (canvas.width*.5 + r1), true) - this.Transform.RelativePosition.x));
	...
	this.PreUpdate = function() 
	{
		if (this.enabled) 
		{
			...
			/*ctx.fillStyle = "green";
			ctx.beginPath()
			ctx.arc(this.Transform.Position.x,this.Transform.Position.y,this.Transform.Size.x * this.Transform.Scale.x * 0.4, 0, Math.PI * 2);
			ctx.closePath();
			ctx.fill();
			ctx.stroke();*/
	...
	{
		this.Transform.RelativePosition.x += this.speed * Time.deltaTime * Math.cos(Math.DegreeToRadian(this.Transform.angle));
		this.Transform.RelativePosition.y += this.speed * Time.deltaTime * Math.sin(Math.DegreeToRadian(this.Transform.angle));

		//this.Transform.RelativePosition.x += this.speed * Time.deltaTime;
		//this.Physics.Collider = new Circle(this.Transform.RelativePosition.x + this.Transform.Size.x * this.Transform.Pivot.x, this.Transform.RelativePosition.y + this.Transform.Size.y * this.Transform.Pivot.y, 50);
	...
	// 	{
	// 		var length = Application.LoadedScene.GameObjects.length;
	// 		// console.log(Ap + "Aste")
	// 		for (var i = 1; i < length; i++) 
	// 		{
				
	// 			// var ParticuleAsteroid = new Kaboom(new Vector(Application.LoadedScene.GameObjects[i].Transform.RelativePosition.x, Application.LoadedScene.GameObjects[i].Transform.RelativePosition.y));

	// 		}

			
	// 		// this.Kaboom.addElement( new Emitter(
	// 		// new Vector(this.Transform.RelativePosition.x,this.Transform.RelativePosition.y),
	// 		// new Vector(5,5),
	// 		// 10,
	// 		// 1,
	// 		// 50))
