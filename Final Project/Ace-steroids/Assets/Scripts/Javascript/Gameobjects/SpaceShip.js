 ...
 *
 * */
function SpaceShip(_anchorPosition) 
{
	this.name = "SpaceShip";
	this.enabled = true;
	this.started = false;
	...
	this.Transform = {};
	this.Transform.RelativePosition = new Vector(-50,_anchorPosition);
	this.Transform.Position = new Vector(-50,_anchorPosition);
	this.Transform.Size = new Vector(613,577);
	this.Transform.RelativeScale = new Vector(1,1);
	this.Transform.Scale = new Vector(.5,.5);
	...
	this.Start = function() 
	{
		if(!this.started)
		{
			// operation start
			// ctx.fillStyle = 'blue';
			// ctx.fillRect(this.Transform.RelativePosition.x, this.Transform.RelativePosition.y, 25, 25);
			this.Renderer.Material.Source = Images["Ship"];
	...
	this.Update = function() 
	{
		...
		{
			this.Transform.RelativePosition.x += this.speed * Time.deltaTime;
			this.Transform.RelativePosition.y = (this.anchorPosition) + Math.sin(Math.DegreeToRadian(this.count))*50;
		}

		this.count += this.speed * Time.deltaTime;

		this.ColissionWithAsteroid();
	...
	this.ColissionWithAsteroid = function()
	{
		for (var i = 0; i < Application.LoadedScene.GameObjects.length; i++) 
		{
			var Go = Application.LoadedScene.GameObjects[i];
			// console.log("asteroid :",Application.LoadedScene.GameObjects[i].Transform.Position);
			// console.log("ship :", this.Transform.Position)

			var boxShip = new Box(this.Transform.Position.x - this.Transform.Size.x * this.Transform.Scale.x * this.Transform.Pivot.x,
									this.Transform.Position.y - this.Transform.Size.y * this.Transform.Scale.y * this.Transform.Pivot.y,
									this.Transform.Size.x * this.Transform.Scale.x ,this.Transform.Size.y * this.Transform.Scale.y);
						
			var boxAsteroids = new Circle(Go.Transform.Position.x,
										Go.Transform.Position.y,
										Go.Transform.Size.x * Go.Transform.Scale.x *0.4); 

			if ( Physics.CircleBoxCollision(boxAsteroids, boxShip) ) 
			{
